...
async function changeMinasBalance(productId: string, delta: number): Promise<void> {
  const currentRows = await db
    .select({ quantity: inventoryMinasBalancesTable.quantity })
    .from(inventoryMinasBalancesTable)
    .where(eq(inventoryMinasBalancesTable.productId, productId))
    .limit(1);
  ...
  if (!currentRows[0]) {
    await db.insert(inventoryMinasBalancesTable).values({
      productId,
      quantity: next,
      updatedAt: new Date(),
    });
  ...
  await db
    .update(inventoryMinasBalancesTable)
    .set({ quantity: next, updatedAt: new Date() })
    .where(eq(inventoryMinasBalancesTable.productId, productId));
}

export async function getMinasStockMap(productIds: string[]): Promise<Map<string, number>> {
  if (productIds.length === 0) return new Map();
  const rows = await db
    .select({ productId: inventoryMinasBalancesTable.productId, quantity: inventoryMinasBalancesTable.quantity })
    .from(inventoryMinasBalancesTable)
    .where(inArray(inventoryMinasBalancesTable.productId, productIds));

  return new Map(rows.map((row) => [row.productId, Number(row.quantity) || 0]));
}

export async function registerMinasInventoryEntry(params: {
  productId: string;
  quantity: number;
  reason?: string;
  referenceId?: string;
  ...
  if (params.affectBalance !== false) {
    await changeMinasBalance(params.productId, params.quantity);
  }
