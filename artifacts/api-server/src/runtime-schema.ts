...
async function ensureOrdersColumns(databaseName: string): Promise<void> {
  ...
  const definitions = [
    ...
    { name: "store_credit_from_edit", sql: "ALTER TABLE orders ADD COLUMN store_credit_from_edit DECIMAL(10,2) NULL" },
    { name: "store_credit_withheld_from_edit", sql: "ALTER TABLE orders ADD COLUMN store_credit_withheld_from_edit DECIMAL(10,2) NULL" },
    {
      name: "observation_visible_to_customer",
      sql: "ALTER TABLE orders ADD COLUMN observation_visible_to_customer TINYINT(1) NOT NULL DEFAULT 0",
    },
    {
...
async function ensureInventoryTables(databaseName: string): Promise<void> {
  ...
  if (!(await tableExists("inventory_motoboy_balances", databaseName))) {
    await pool.query(`
      CREATE TABLE inventory_motoboy_balances (
        product_id VARCHAR(255) NOT NULL PRIMARY KEY,
        quantity INT NOT NULL DEFAULT 0,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
      )
  ...
  if (!(await tableExists("inventory_motoboy_movements", databaseName))) {
    await pool.query(`
      CREATE TABLE inventory_motoboy_movements (
        id VARCHAR(255) NOT NULL PRIMARY KEY,
        product_id VARCHAR(255) NOT NULL,
        type VARCHAR(32) NOT NULL DEFAULT 'entry',
        entry_source VARCHAR(32) NULL,
        ...
        reference_id VARCHAR(255) NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        KEY inventory_motoboy_movements_product_id_idx (product_id),
        KEY inventory_motoboy_movements_type_idx (type),
        KEY inventory_motoboy_movements_created_at_idx (created_at)
      )
    `);
  }
