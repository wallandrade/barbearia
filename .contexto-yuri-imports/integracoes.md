...
|------|--------|---------|-----------------|
| 2026-08-27 | Chat de compostos: OpenAI opcional; sem chave responde pelas fichas (`answerFromPeptideKnowledge`) | Bolha não depende mais da chave no Railway | OCR de etiqueta igual |
...
- **Chat informativo (compostos):** `OPENAI_API_KEY` **opcional**. Sem chave (ou se OpenAI falhar), responde com match nome/apelido nas fichas (`answerFromPeptideKnowledge`). Com chave: `POST https://api.openai.com/v1/chat/completions` (`OPENAI_CHAT_MODEL` ou `OPENAI_VISION_MODEL` / `gpt-4o-mini`; temp 0,2). Rotas `GET /api/chat/status` (`enabled: true`) e `POST /api/chat/ask` (rate 20/10 min/IP; **sem** checkout token). Não substitui médico; não confirma PIX/pedido.
