...
- Opções de frete: `shipping_options`; seguro de envio na aba Admin **Seguro** (settings `checkout_insurance_*` + `checkout_insurance_keep_percent`). Opcional no checkout. Snapshot no pedido: `insurance_keep_amount` / `insurance_cashback_amount`. Se **entregue** EnvioEcom e tem `userId` e sem sinistro: credita saldo (`insurance_cashback`). 1ª perda: reenvio 1x (frete do filho R$ 0) **ou** estorno do **subtotal** em saldo (seguro não volta). 2ª perda do reenvio: estorna produto, não cria 3º envio. Sem seguro: chamado `extravio` **não** reenvia. “Veio faltando” continua reenviando. % especial por produto igual. Default cobrado **10%**; % da loja default **10%** (saldo 0 até você subir o cobrado). Desativado some o checkbox; create/PIX recalcula e ignora `insuranceAmount` do cliente. Carteira: `GET /api/me/store-credit`, uso no checkout `useStoreCredit`. Frete grátis por valor mínimo: settings `checkout_free_shipping_min_subtotal` (envio padrão) e `checkout_free_shipping_min_motoboy` (Motoboy); checkout/API escolhem pelo `shippingType` / opção selecionada. Em branco = desativado naquele frete.
...
- **Abertura (cliente):** após escolher o pedido, escolhe **Pedido veio faltando**, **Não chegou / extravio** ou **Outro problema**. Extravio chama `markFirstLost`. Sem seguro o admin **não** reenvia (API `NO_INSURANCE`); com seguro o Reenviar usa `chooseInsuranceReship` (máx. 1). “Veio faltando” continua reenviando itens.
