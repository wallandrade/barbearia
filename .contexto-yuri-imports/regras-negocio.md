...
**Não misturar:** Aguardando coleta/postagem = etiqueta pronta (cópia). **Não** marca Enviado e **não** baixa estoque. Enviado/baixa = Coleta Recebida / coletado / trânsito **ou** clique **Marcar como Enviado**. No split, baixa é **por pacote** (pool + itens daquele envio).
...
- **Estoque Motoboy** (pool independente): `inventory_motoboy_balances` / `inventory_motoboy_movements`; aba Admin Estoque → Motoboy (entrada/saída manual). No card, **Motoboy** só grava `inventory_pool` (**sem** baixa na escolha); a baixa acontece em **Marcar Enviado**, **Dar baixa agora** (`reserveNow` → `inventory_reserved`) **ou etiqueta EnvioEcom**. **Loja** continua reservando na escolha. **EnvioEcom SP** (`id=env` / São Paulo) **sempre** baixa Motoboy (etiqueta, Coleta Recebida, Marcar Enviado se o pedido tiver essa conta). **EnvioEcom MG** (conta extra) **sempre** baixa Minas. A pílula Foz não decide a baixa EE. A faixa Foz/Motoboy/Minas **permanece no card Enviado** para baixa atrasada se `inventory_reserved` ainda for falso. Com reserva feita, Coletado EE / Marcar Enviado / etiqueta **não** baixam de novo; o card mostra **Baixa feita: OK** e troca de pool exige **Marcar como Pendente**. Entrada Motoboy **não** debita a loja automaticamente.
- **Estoque Minas** (pool independente, mesmo padrão Motoboy): `inventory_minas_balances` / `inventory_minas_movements`; aba Admin Estoque → Minas (entrada/saída manual). No card, **Minas** só grava `inventory_pool` (**sem** baixa na escolha); a baixa acontece em **Marcar Enviado**, **Dar baixa agora** **ou etiqueta EnvioEcom**. **Conta EnvioEcom MG** baixa sempre este pool. Entrada Minas **não** debita Loja nem Motoboy. Reenvio manual / produto voltando ficam só na aba Loja. Pedido **dividido**: cada pacote baixa pelo pool da **conta EE** daquele envio (SP→Motoboy, MG→Minas); o seletor Loja/Motoboy/Minas do card some.
...
- **Etiqueta EnvioEcom:** ao gerar (PDF/R2 ou download local) **dá baixa** no pool da **conta EE** (SP → Motoboy, MG → Minas) e marca `inventory_reserved`. Sem conta EE, cai no pool do card (ou Foz). Status vira **Etiqueta emitida** se ainda não estava em trânsito. Sem saldo: etiqueta sai mesmo assim, grava o pool da conta e o toast avisa. Cópia 48h / Enviado: ver **Invariante — cópia 48h** no topo deste arquivo (não anular no changelog). Split com **parte** etiquetada: badge **Enviado parcialmente**; cópia de envio (48h/Resumo/lista) **omite** os itens já com etiqueta. Copiar Completo e pós-pagamento não filtram.
