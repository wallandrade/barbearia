...
**Não misturar:** Aguardando coleta/postagem = etiqueta pronta (cópia). **Não** marca Enviado e **não** baixa estoque. Enviado = Coleta Recebida / coletado / trânsito **ou** clique **Marcar como Enviado**. **Baixa de estoque só no clique Dar baixa agora** (`reserveNow`). Etiqueta EE, webhook/sync e Marcar Enviado **não** descontam. No split, baixa é **por pacote** (botão na linha da origem).
...
|------|--------|---------|-----------------|
| 2026-09-09 | `POST /api/integrations/inventory/exit` recusa (403 `EXIT_DISABLED`). Espelho Motoboy/Minas só lê o snapshot | Outro sistema não desconta estoque Yury | GET snapshot, webhook `inventory.changed` e Dar baixa agora iguais |
...
- **Sync estoque Motoboy+Minas (espelho):** `GET /api/integrations/inventory/snapshot` (token `INVENTORY_SYNC_TOKEN` ou `MOTOBOY_SYNC_TOKEN`) é **só leitura**. `POST /api/integrations/inventory/exit` responde 403 `EXIT_DISABLED` — baixa só no Admin (**Dar baixa agora**). Webhook opcional `inventory.changed` nas saídas do Admin. Sem entrada de compra por essa API. Foz Guaçu não entra.
