...
**Envio dividido** (2+ pacotes em `order_shipments`): a tabela acima vale **por pacote**. O pedido só sai da cópia quando **todos** os pacotes já teriam saído. Um pacote Minas com PDF **não** remove o Motoboy ainda sem etiqueta.
...
**Proibido em changelog/código:** “Aguardando coleta não conta” para a **cópia**. Funções: `isExcludedFromShippingCopyList` + `isEnvioEcomLabelReadyStatus` (`Admin.tsx`); `isSplitOrderExcludedFromShippingCopyList` (`order-shipments-logic.ts`); `isLabelReadyStatus` / `isInTransitStatus` (`lib/envioecom.ts`). Teste: `envioecom-status.test.ts` + `order-shipments.test.ts`.
