...
- Catálogo (todas as categorias): ordem **mais vendido → menos vendido** (unidades em pedidos `paid`/`completed`, sem pedido filho de reenvio). Sem badge/texto de ranking no card. Em Peptídeo isso vale **dentro** de cada marca. `soldQty` vai no JSON da API só para ordenar, não aparece na UI.
...
- Inventário loja (UI **Foz Guaçu**, chave `loja`): saldos e movimentos (`inventory_balances` / `inventory_movements`), retornos manuais (`manual_return_items`). Nome na lista vem do catálogo (`products.id`); foto no Admin vem do cadastro, não do overview.
