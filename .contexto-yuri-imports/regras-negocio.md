...
- **Compra com fornecedor** (aba Despesas): cadastro de fornecedor; rascunho tipo carrinho (produto, qtd, custo); **Travar pedido** (`ordered`); **Dar entrada no estoque** (admin primário) escolhe pool Foz Guaçu/Motoboy/Minas, dá entrada, atualiza `costPrice` se mudou e lança despesa `compra_fornecedor` **paga** no **dia civil de Brasília** (00:00–23:59 −03). GET da lista repara compra concluída sem linha de despesa e alinha as datas ao filtro. UI: recibo com selos Paga/estoque; lançamento avulso (extravio/reenvio) fica recolhido. Tabelas `suppliers` / `supplier_purchases` / `supplier_purchase_items`.
