...
- **Extrato OFX (Inter):** aba Admin **Extrato** — upload `.ofx` → `POST /api/admin/bank-statement/analyze` (créditos PIX) → relatório; **Aplicar só 100%** grava `bank_deposit_match_status=confirmed_100` (badge Depósito 100%); demais matches/`apply` geral usam `ok` | `not_found`. Match: valor exato + janela de dias + nome; ambíguos manuais. **Não** altera `status` pago automaticamente.
