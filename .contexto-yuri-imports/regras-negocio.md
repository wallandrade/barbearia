...
|------|--------|---------|-----------------|
| 2026-09-02 | Motoboy por km: CEP origem (Sé) + faixas; cascata bairro → km → faixa CEP; >200 km consulta (não cai na faixa) | Checkout cotação por distância (BrasilAPI + Haversine) | Frete padrão R$50; bairros e faixas CEP continuam |
...
- Motoboy: bairros (`motoboy_neighborhoods`) + **km a partir do CEP de partida** (settings `motoboy_distance_*`, default Sé `01001000`) + faixas de CEP (`motoboy_cep_ranges`) + slots/bookings — `motoboy-*` + `GET /api/motoboy-coverage/lookup`.
  - Ordem no checkout: bairro ativo → cotação km (se ligada) → faixa CEP. Sé/centro/República em SP ou CEP `010xxxxx` = preço de centro (default R$ 50); demais km pelas faixas (10→70, 15→80, 30→100, 50→120, 80→150, 120→200, 200→300). Acima do limite (default 200 km) **não oferece Motoboy** (consultar pessoalmente) e **não** usa faixa CEP.
  - Slots: id `dist` usa intervalo 2h. Km usa coordenadas BrasilAPI CEP v2 + Haversine; CEP sem coords cai na faixa CEP.
