...
- **Extrato OFX (Inter):** aba Admin **Extrato** — upload `.ofx` → `POST /api/admin/bank-statement/analyze` (créditos PIX) → relatório match/ambíguo/sem pedido; `apply` grava `bank_deposit_match_status` (`ok` | `not_found`) + FITID/valor/nome/data. Match: **valor centavo a centavo**, janela de dias (padrão 5), nome do pagador; ambíguos exigem escolha manual. **Não** altera `status` pago automaticamente.
