...
**Não misturar:** Aguardando coleta/postagem = etiqueta pronta (cópia). **Não** marca Enviado e **não** baixa estoque. Enviado = Coleta Recebida / coletado / trânsito **ou** clique **Marcar como Enviado**. **Baixa no Admin:** só **Dar baixa agora** (`reserveNow`). Etiqueta EE, webhook/sync e Marcar Enviado **não** descontam. No split, baixa é **por pacote**. **Espelho Motoboy/Minas:** `POST /integrations/inventory/exit` baixa se a senha abriu a janela de 10 min.
...
| 2026-09-10 | Espelho Motoboy/Minas volta a baixar via API **com senha**: janela de 10 min; depois pede de novo. Admin Configurações cadastra/troca (hash). Snapshot continua livre | Outro sistema não desconta sozinho o tempo todo | Dar baixa agora no card, EE sem baixa automática |
